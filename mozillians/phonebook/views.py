...
def edit_profile(request):
    ...
    if (user_form.is_valid() and profile_form.is_valid()):
        ...
        return redirect(reverse('phonebook:profile', args=[user.username]))
...
def search(request):
    ...
    if form.is_valid():
        ...
        if profiles.count() == 1 and not groups:
            return redirect('phonebook:profile', people[0].user.username)
...
def invite(request):
    ...
    if request.method == 'POST' and invite_form.is_valid():
        ...
        return redirect('phonebook:profile', request.user.username)
...
def vouch(request):
    ...
    if form.is_valid():
        ...
        return redirect('phonebook:profile', p.user.username)
